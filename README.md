...
1. **Препроцессинг** (`preprocessing/`)
   ...
   - Улучшение качества изображений для лучшего распознавания
   - Удаление фона, шумоподавление, повышение контраста
   - Поддержка различных пресетов для разных типов документов

2. **Извлечение данных** (`extraction/`)

   - Валидация и подготовка изображений
   - Извлечение структурированных данных через Groq Vision API
   - Парсинг и валидация JSON-ответов

3. **Векторизация** (`vectorization/`)

   - Создание векторных представлений документов (SBERT)
   - Индексация в ChromaDB для семантического поиска

4. **Генерация** (`generation/`)

   - Построение контекста из различных источников данных
   - Генерация ответов через LLM с использованием RAG

5. **Хранение данных** (`repository/`)

   - SQLite для базовой информации о пациентах
   - MongoDB для детальных медицинских записей
   - ChromaDB для векторных представлений

6. **Генерация отчётов** (`reporting/`)
   - Форматирование данных в Markdown
   - Конвертация в PDF с использованием WeasyPrint
